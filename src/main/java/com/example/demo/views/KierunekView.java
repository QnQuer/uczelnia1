...
public class KierunekView extends HorizontalLayout implements View{
     ...
     Button addButton = new Button("Add");
        ...
        verticalLayout.addComponents(przedmiotComboBox, horizontalTextFieldLayout, horizontallAddAndDeleteButtonLayout, grid);
         ...
         przedmiotComboBox.addSelectionListener(select -> {
            setGridElements(grid, kierunekRepository.findAllByPrzedmiot(select.getValue()));
        });
         addButton.addClickListener (click -> {
             try {
                if (nazwa.getValue().length() > 0  && przedmiotComboBox.getSelectedItem().get().getPrzedmiotId() != null) {
                    ...
                    Kierunek.setPrzedmiot(przedmiotComboBox.getSelectedItem().get());
                    
                    przedmiotRepository.save(kierunek);
                    setGridElements(grid, kierunekRepository.findAllByPrzedmiot(przedmiotComboBox.getSelectedItem().get()));
         ...
         updateButton.addClickListener(click -> {
            ...
            if (grid.getSelectionModel().getFirstSelectedItem().isPresent()) {
                Kierunek kierunek = kierunekRepository.findOneByKierunekId(grid.getSelectionModel().getFirstSelectedItem().get().getId());
                kierunek.setNazwa(nazwa.getValue());
    ...
    /**
     * Method called onice we ENTER the site (which mean no refresh, just enter or switch (using side menu for example).
     * @param event
     */
    @Override
